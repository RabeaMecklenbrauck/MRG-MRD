...
```{r}
df<- read_xlsx("/Users/maximilianbader/Library/Mobile Documents/com~apple~CloudDocs/Universitaet/AG Heuser/analysis/2025/MRDMRGMshared/Export data for R/250807_MRGM-Data_Analysis_v41.2_anonym.xlsx", na = "na")
```
...
t2cohort_imp<-t2tx%>%select(OS_status, OS_fromTx_years_censored, Hb_log, WBC_log, Plt_log, age_cat,KT_complex, NPM1_code, t2_MRD_MRGM_positive1_negative0, Sex, AML_Type)
...
coxph(Surv(OS_fromTx_years_censored, OS_status)~ AML_Type, data = imputed) %>% tbl_regression (exp = TRUE)
```
```{r}
...
MVA RFS
```{r}
t2cohort_imp<-t2tx%>%select(RFS_status, RFS_fromTx_years_censored, Hb_at_ED, WBC_at_ED, Platelets_at_ED, Age_at_ED,KT_complex, NPM1_code, t2_MRD_MRGM_positive1_negative0, Sex, AML_Type)
...
```{r}
t2tx$age_cat<-as.numeric(t2tx$age_cat)
t2cohort_imp<-t2tx%>%select(RFS_status, RFS_fromTx_years_censored, Hb_log, WBC_log, Plt_log, age_cat,KT_complex, NPM1_code, t2_MRD_MRGM_positive1_negative0, Sex, AML_Type)
t2cohort_imp<-t2cohort_imp%>%drop_na(RFS_fromTx_years_censored)
...
coxph(Surv(RFS_fromTx_years_censored, RFS_status)~ AML_Type, data = imputed) %>% tbl_regression (exp = TRUE)
