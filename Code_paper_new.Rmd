...
```{r}
df<- read_xlsx("/Users/maximilianbader/Library/Mobile Documents/com~apple~CloudDocs/Universitaet/AG Heuser/analysis/2025/MRDMRGMshared/Export data for R/250802_MRGM-Data_Analysis_v37_anonym.xlsx", na = "na")
```
...
OS t2tx
```{r}
fit_t2tx<- survfit(Surv(OS_years_censored, OS_status)~t2tx$t2_MRD_MRGM_positive1_negative0, data=t2tx)
ggsurv <- ggsurvplot(
...
```{r}
survdiff(Surv(OS_years_censored, OS_status)~t2tx$t2_MRD_MRGM_positive1_negative0, data=t2tx)
coxph(Surv(OS_years_censored, OS_status) ~ t2tx$t2_MRD_MRGM_positive1_negative0, data=t2tx) %>% 
...
```{r}
library(mice)
t2cohort_imp<-t2tx%>%select(OS_status, OS_years_censored, Hb_at_ED, WBC_at_ED, Platelets_at_ED, Age_at_ED,KT_complex, NPM1_code, t2_MRD_MRGM_positive1_negative0)
...
coxph(Surv(OS_years_censored, OS_status)~Hb_at_ED + WBC_at_ED +Platelets_at_ED + Age_at_ED  + KT_complex +imputed$NPM1_code + t2_MRD_MRGM_positive1_negative0, data = imputed) %>% tbl_regression (exp = TRUE)
```
```{r}
coxph(Surv(OS_years_censored, OS_status)~Platelets_at_ED + t2_MRD_MRGM_positive1_negative0, data = imputed) %>% tbl_regression (exp = TRUE)
```
MVA step up
```{r}
coxph(Surv(OS_years_censored, OS_status)~Hb_at_ED , data = imputed) %>% tbl_regression (exp = TRUE)
coxph(Surv(OS_years_censored, OS_status)~WBC_at_ED , data = imputed) %>% tbl_regression (exp = TRUE)

```
```{r}
coxph(Surv(OS_years_censored, OS_status)~Platelets_at_ED, data = imputed) %>% tbl_regression (exp = TRUE)
coxph(Surv(OS_years_censored, OS_status)~Age_at_ED , data = imputed) %>% tbl_regression (exp = TRUE)
coxph(Surv(OS_years_censored, OS_status)~ KT_complex, data = imputed) %>% tbl_regression (exp = TRUE)
coxph(Surv(OS_years_censored, OS_status)~NPM1_code , data = imputed) %>% tbl_regression (exp = TRUE)
coxph(Surv(OS_years_censored, OS_status)~t2_MRD_MRGM_positive1_negative0, data = imputed) %>% tbl_regression (exp = TRUE)
```
...
t2cohort_imp<-t2tx%>%select(OS_status, OS_years_censored, Hb_log, WBC_log, Plt_log, age_cat,KT_complex, NPM1_code, t2_MRD_MRGM_positive1_negative0)
...
coxph(Surv(OS_years_censored, OS_status)~Hb_log + WBC_log +Plt_log + age_cat  + KT_complex +imputed$NPM1_code + t2_MRD_MRGM_positive1_negative0, data = imputed) %>% tbl_regression (exp = TRUE)
```

```{r}
coxph(Surv(OS_years_censored, OS_status)~Plt_log  + t2_MRD_MRGM_positive1_negative0, data = imputed) %>% tbl_regression (exp = TRUE)
```
```{r}
coxph(Surv(OS_years_censored, OS_status)~Hb_log , data = imputed) %>% tbl_regression (exp = TRUE)
coxph(Surv(OS_years_censored, OS_status)~ WBC_log , data = imputed) %>% tbl_regression (exp = TRUE)
coxph(Surv(OS_years_censored, OS_status)~Plt_log, data = imputed) %>% tbl_regression (exp = TRUE)
coxph(Surv(OS_years_censored, OS_status)~age_cat, data = imputed) %>% tbl_regression (exp = TRUE)
coxph(Surv(OS_years_censored, OS_status)~ KT_complex, data = imputed) %>% tbl_regression (exp = TRUE)
coxph(Surv(OS_years_censored, OS_status)~imputed$NPM1_code , data = imputed) %>% tbl_regression (exp = TRUE)
coxph(Surv(OS_years_censored, OS_status)~ t2_MRD_MRGM_positive1_negative0, data = imputed) %>% tbl_regression (exp = TRUE)
```
```{r}
coxph(Surv(OS_years_censored, OS_status)~ Plt_log + t2_MRD_MRGM_positive1_negative0, data = imputed) %>% tbl_regression (exp = TRUE)
...
RFS t2tx
```{r}
fit_t2tx<- survfit(Surv(RFS_years_censored, RFS_status)~t2tx$t2_MRD_MRGM_positive1_negative0, data=t2tx)
ggsurv <- ggsurvplot(
  fit_t2tx,                     # survfit object with calculated statistics.
  data = t2tx,             # data used to fit survival curves.
  risk.table = TRUE, fontsize = 5,      # show risk table.
...
Calculate the p value with logrank test and Cox regression.
```{r}
survdiff(Surv(RFS_years_censored, RFS_status)~t2tx$t2_MRD_MRGM_positive1_negative0, data=t2tx)
coxph(Surv(RFS_years_censored, RFS_status) ~ t2tx$t2_MRD_MRGM_positive1_negative0, data=t2tx) %>% 
  tbl_regression(exp = TRUE)
```

MVA RFS
```{r}
t2cohort_imp<-t2tx%>%select(RFS_status, RFS_years_censored, Hb_at_ED, WBC_at_ED, Platelets_at_ED, Age_at_ED,KT_complex, NPM1_code, t2_MRD_MRGM_positive1_negative0)
t2cohort_imp<-t2cohort_imp%>%drop_na(RFS_years_censored)
init = mice(t2cohort_imp, maxit=0) 
meth = init$method
predM = init$predictorMatrix
...
coxph(Surv(RFS_years_censored, RFS_status)~Hb_at_ED + WBC_at_ED +Platelets_at_ED + Age_at_ED  + KT_complex +imputed$NPM1_code + t2_MRD_MRGM_positive1_negative0, data = imputed) %>% tbl_regression (exp = TRUE)
```

RFS with age in decades and log transformed
```{r}
t2tx$age_cat<-as.numeric(t2tx$age_cat)
t2cohort_imp<-t2tx%>%select(RFS_status, RFS_years_censored, Hb_log, WBC_log, Plt_log, age_cat,KT_complex, NPM1_code, t2_MRD_MRGM_positive1_negative0)
t2cohort_imp<-t2cohort_imp%>%drop_na(RFS_years_censored)
...
coxph(Surv(RFS_years_censored, RFS_status)~Hb_log + WBC_log +Plt_log + age_cat  + KT_complex +imputed$NPM1_code + t2_MRD_MRGM_positive1_negative0, data = imputed) %>% tbl_regression (exp = TRUE)
```
...
CIR t2tx
```{r}
ci.all<-cuminc(ftime=t2tx$RFS_years_censored,fstatus=t2tx$CIR_event,group=t2tx$t2_MRD_MRGM_positive1_negative0)
print(ci.all)
...
fine_gray_test <- crr(t2tx$RFS_years_censored, t2tx$CIR_event == 1, t2tx$t2_MRD_MRGM_positive1_negative0)
...
Supplemental Figure2: OS, RFS and CIR with ASXL1 excluded
OS t2tx without ASXL1
```{r}
fit_t2tx<- survfit(Surv(OS_years_censored, OS_status)~t2tx$t2_MRD_MRGM_noASXL1, data=t2tx)
ggsurv <- ggsurvplot(
  fit_t2tx,                     # survfit object with calculated statistics.
  data = t2tx,             # data used to fit survival curves.
  risk.table = TRUE, fontsize = 5,      # show risk table.
...
Calculate the p value with logrank test and Cox regression.
```{r}
survdiff(Surv(OS_years_censored, OS_status)~t2tx$t2_MRD_MRGM_noASXL1, data=t2tx)
coxph(Surv(OS_years_censored, OS_status) ~ t2tx$t2_MRD_MRGM_noASXL1, data=t2tx) %>% 
  tbl_regression(exp = TRUE)
```
RFS t2tx
```{r}
fit_t2tx<- survfit(Surv(RFS_years_censored, RFS_status)~t2tx$t2_MRD_MRGM_noASXL1, data=t2tx)
ggsurv <- ggsurvplot(
  fit_t2tx,                     # survfit object with calculated statistics.
  data = t2tx,             # data used to fit survival curves.
  risk.table = TRUE, fontsize = 5,      # show risk table.
...
Calculate the p value with logrank test and Cox regression.
```{r}
survdiff(Surv(RFS_years_censored, RFS_status)~t2tx$t2_MRD_MRGM_noASXL1, data=t2tx)
coxph(Surv(RFS_years_censored, RFS_status) ~ t2tx$t2_MRD_MRGM_noASXL1, data=t2tx) %>% 
  tbl_regression(exp = TRUE)
```
CIR t2tx
```{r}
ci.all<-cuminc(ftime=t2tx$RFS_years_censored,fstatus=t2tx$CIR_event,group=t2tx$t2_MRD_MRGM_noASXL1)
print(ci.all)
# select subset only CIR, no NRM [3:4]
fig<- ggcompetingrisks( fit =ci.all,
gnames = NULL,
...
fine_gray_test <- crr(t2tx$RFS_years_censored, t2tx$CIR_event == 1, t2tx$t2_MRD_MRGM_noASXL1)
summary(fine_gray_test)
