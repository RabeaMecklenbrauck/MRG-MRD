...
CIR
```{r}
# load competing risk package (may need to install first):
install.packages("cmprsk")
library(cmprsk)


ci.all<-cuminc(ftime=MRD_MRGM$RFS_years,fstatus=MRD_MRGM$CIR_event,group=MRD_MRGM$t1_MRD_MRGM_positive1_negative0)
print(ci.all)
...
xlab = "Years", 
ylab = "Probability")
fig<-fig+scale_color_manual(name="", values=c("#20854E","#BC3C29"), labels=c("MRD neg", "MRD pos"))
...
fine_gray_test <- crr(MRD_MRGM$RFS_years, MRD_MRGM$CIR_event == 1, MRD_MRGM$t1_MRD_MRGM_positive1_negative0)
...
OS t2cohort
```{r}
fit_t2cohort<- survfit(Surv(OS_years, OS_status)~t2cohort$t1_MRD_MRGM_positive1_negative0, data=t2cohort)
ggsurv <- ggsurvplot(
...
Calculate the p value with logrank test and Cox regression.
```{r}
survdiff(Surv(OS_years, OS_status)~t2cohort$t1_MRD_MRGM_positive1_negative0, data=t2cohort)
coxph(Surv(OS_years, OS_status) ~ t2cohort$t1_MRD_MRGM_positive1_negative0, data=t2cohort) %>% 
  tbl_regression(exp = TRUE)
```
RFS t2cohort
```{r}
