...
t1compositivnodta=subset(df, df$`Einschluss t1_oder_t2`==1 & df$t1_MRD_Comutations_noDTA==1)
t1comnegativnodta=subset(df, df$`Einschluss t1_oder_t2`==1 & df$t1_MRD_Comutations_noDTA==0)
t2compositivnodta=subset(df, df$`Einschluss t1_oder_t2`==1 & df$t2_MRD_Comutations_noDTA==1)
t2comnegativnodta=subset(df, df$`Einschluss t1_oder_t2`==1 & df$t2_MRD_Comutations_noDTA==0)
...
Supplemental table 2.1: Co-mutations t1cohort > code see above
```{r}
df = t1cohort
table(df$ASXL2_code)
table(df$ABCA1_code)
...
Supplemental table 2.2: Co-mutations t2cohort > code see above
```{r}
df = t2cohort
table(df$ASXL2_code)
table(df$ABCA1_code)
...
OS t1compositivnodta -> t1, Comutations MRD+ without DTA-mutations, MRGM MRD pos vs. neg.
```{r}
fit_t1cohort<- survfit(Surv(OS_years, OS_status)~t1compositivnodta$t1_MRD_MRGM_positive1_negative0, data=t1compositivnodta)
ggsurv <- ggsurvplot(
  fit_t1cohort,                     # survfit object with calculated statistics.
  data = t1compositivnodta,             # data used to fit survival curves.
  risk.table = TRUE, fontsize = 5,      # show risk table.
  pval = TRUE, # show p-value of log-rank test.
  pval.coord = c(0.1, 0.04), #P-value position
  conf.int = FALSE,         # show confidence intervals for 
...
Calculate the p value with logrank test and Cox regression.
```{r}
survdiff(Surv(OS_years, OS_status)~t1compositivnodta$t1_MRD_MRGM_positive1_negative0, data=t1compositivnodta)
coxph(Surv(OS_years, OS_status) ~ t1compositivnodta$t1_MRD_MRGM_positive1_negative0, data=t1compositivnodta) %>% 
  tbl_regression(exp = TRUE)
```
RFS t1compositivnodta -> t1, Comutations MRD+ without DTA, MRGM MRD pos vs. neg.
```{r}
fit_t1cohort<- survfit(Surv(RFS_years, RFS_status)~t1compositivnodta$t1_MRD_MRGM_positive1_negative0, data=t1compositivnodta)
ggsurv <- ggsurvplot(
  fit_t1cohort,                     # survfit object with calculated statistics.
  data = t1compositivnodta,             # data used to fit survival curves.
  risk.table = TRUE, fontsize = 5,      # show risk table.
...
Calculate the p value with logrank test and Cox regression.
```{r}
survdiff(Surv(RFS_years, RFS_status)~t1compositivnodta$t1_MRD_MRGM_positive1_negative0, data=t1compositivnodta)
coxph(Surv(RFS_years, RFS_status) ~ t1compositivnodta$t1_MRD_MRGM_positive1_negative0, data=t1compositivnodta) %>% 
  tbl_regression(exp = TRUE)
```
CIR t1compositivnodta -> t1, Comutations without DTA MRD+, MRGM MRD pos vs. neg.
```{r}
ci.all<-cuminc(ftime=t1compositivnodta$RFS_years,fstatus=t1compositivnodta$CIR_event,group=t1compositivnodta$t1_MRD_MRGM_positive1_negative0)
print(ci.all)
# select subset only CIR, no NRM [3:4]
fig<- ggcompetingrisks( fit =ci.all,
gnames = NULL,
...
fine_gray_test <- crr(t1compositivnodta$RFS_years, t1compositivnodta$CIR_event == 1, t1compositivnodta$t1_MRD_MRGM_positive1_negative0)
summary(fine_gray_test)

```


OS t1comnegativnodta -> t1, Comutations without DTA-Mutations MRD-, MRGM MRD pos vs. neg.
```{r}
fit_t1cohort<- survfit(Surv(OS_years, OS_status)~t1comnegativnodta$t1_MRD_MRGM_positive1_negative0, data=t1comnegativnodta)
ggsurv <- ggsurvplot(
  fit_t1cohort,                     # survfit object with calculated statistics.
  data = t1comnegativnodta,             # data used to fit survival curves.
  risk.table = TRUE, fontsize = 5,      # show risk table.
  pval = TRUE, # show p-value of log-rank test.
  pval.coord = c(0.1, 0.04), #P-value position
  conf.int = FALSE,         # show confidence intervals for 
...
Calculate the p value with logrank test and Cox regression.
```{r}
survdiff(Surv(OS_years, OS_status)~t1comnegativnodta$t1_MRD_MRGM_positive1_negative0, data=t1comnegativnodta)
coxph(Surv(OS_years, OS_status) ~ t1comnegativnodta$t1_MRD_MRGM_positive1_negative0, data=t1comnegativnodta) %>% 
  tbl_regression(exp = TRUE)
```
RFS t1comnegativnodta -> t1, Comutations without dta MRD-, MRGM MRD pos vs. neg.
```{r}
fit_t1cohort<- survfit(Surv(RFS_years, RFS_status)~t1comnegativnodta$t1_MRD_MRGM_positive1_negative0, data=t1comnegativnodta)
ggsurv <- ggsurvplot(
  fit_t1cohort,                     # survfit object with calculated statistics.
  data = t1comnegativnodta,             # data used to fit survival curves.
  risk.table = TRUE, fontsize = 5,      # show risk table.
...
Calculate the p value with logrank test and Cox regression.
```{r}
survdiff(Surv(RFS_years, RFS_status)~t1comnegativnodta$t1_MRD_MRGM_positive1_negative0, data=t1comnegativnodta)
coxph(Surv(RFS_years, RFS_status) ~ t1comnegativnodta$t1_MRD_MRGM_positive1_negative0, data=t1comnegativnodta) %>% 
  tbl_regression(exp = TRUE)
```
CIR t1comnegativnodta -> t1, Comutations without dta mutations MRD-, MRGM MRD pos vs. neg.
```{r}
ci.all<-cuminc(ftime=t1comnegativnodta$RFS_years,fstatus=t1comnegativnodta$CIR_event,group=t1comnegativnodta$t1_MRD_MRGM_positive1_negative0)
print(ci.all)
# select subset only CIR, no NRM [3:4]
fig<- ggcompetingrisks( fit =ci.all,
gnames = NULL,
...
fine_gray_test <- crr(t1comnegativnodta$RFS_years, t1comnegativnodta$CIR_event == 1, t1comnegativnodta$t1_MRD_MRGM_positive1_negative0)
summary(fine_gray_test)

```


OS t2compositivnodta -> t2, Comutations without dta-mutations MRD+, MRGM MRD pos vs. neg.
```{r}
fit_t2cohort<- survfit(Surv(OS_years, OS_status)~t2compositivnodta$t2_MRD_MRGM_positive1_negative0, data=t2compositivnodta)
ggsurv <- ggsurvplot(
  fit_t2cohort,                     # survfit object with calculated statistics.
  data = t2compositivnodta,             # data used to fit survival curves.
  risk.table = TRUE, fontsize = 5,      # show risk table.
  pval = TRUE, # show p-value of log-rank test.
  pval.coord = c(0.1, 0.04), #P-value position
  conf.int = FALSE,         # show confidence intervals for 
...
Calculate the p value with logrank test and Cox regression.
```{r}
survdiff(Surv(OS_years, OS_status)~t2compositivnodta$t2_MRD_MRGM_positive1_negative0, data=t2compositivnodta)
coxph(Surv(OS_years, OS_status) ~ t2compositivnodta$t2_MRD_MRGM_positive1_negative0, data=t2compositivnodta) %>% 
  tbl_regression(exp = TRUE)
```
RFS t2compositivnodta -> t2, Comutations without dta mutations MRD+, MRGM MRD pos vs. neg.
```{r}
fit_t2cohort<- survfit(Surv(RFS_years, RFS_status)~t2compositivnodta$t2_MRD_MRGM_positive1_negative0, data=t2compositivnodta)
ggsurv <- ggsurvplot(
  fit_t2cohort,                     # survfit object with calculated statistics.
  data = t2compositivnodta,             # data used to fit survival curves.
  risk.table = TRUE, fontsize = 5,      # show risk table.
...
Calculate the p value with logrank test and Cox regression.
```{r}
survdiff(Surv(RFS_years, RFS_status)~t2compositivnodta$t2_MRD_MRGM_positive1_negative0, data=t2compositivnodta)
coxph(Surv(RFS_years, RFS_status) ~ t2compositivnodta$t2_MRD_MRGM_positive1_negative0, data=t2compositivnodta) %>% 
  tbl_regression(exp = TRUE)
```
CIR t2compositivnodta -> t2, Comutations without dta MRD+, MRGM MRD pos vs. neg.
```{r}
ci.all<-cuminc(ftime=t2compositivnodta$RFS_years,fstatus=t2compositivnodta$CIR_event,group=t2compositivnodta$t2_MRD_MRGM_positive1_negative0)
print(ci.all)
# select subset only CIR, no NRM [3:4]
fig<- ggcompetingrisks( fit =ci.all,
gnames = NULL,
...
fine_gray_test <- crr(t2compositivnodta$RFS_years, t2compositivnodta$CIR_event == 1, t2compositivnodta$t2_MRD_MRGM_positive1_negative0)
summary(fine_gray_test)

```

OS t2comnegativnodta -> t2, Comutations without dta mutations MRD-, MRGM MRD pos vs. neg.
```{r}
fit_t2cohort<- survfit(Surv(OS_years, OS_status)~t2comnegativnodta$t2_MRD_MRGM_positive1_negative0, data=t2comnegativnodta)
ggsurv <- ggsurvplot(
  fit_t2cohort,                     # survfit object with calculated statistics.
  data = t2comnegativnodta,             # data used to fit survival curves.
  risk.table = TRUE, fontsize = 5,      # show risk table.
  pval = TRUE, # show p-value of log-rank test.
  pval.coord = c(0.1, 0.04), #P-value position
  conf.int = FALSE,         # show confidence intervals for 
...
Calculate the p value with logrank test and Cox regression.
```{r}
survdiff(Surv(OS_years, OS_status)~t2comnegativnodta$t2_MRD_MRGM_positive1_negative0, data=t2comnegativnodta)
coxph(Surv(OS_years, OS_status) ~ t2comnegativnodta$t2_MRD_MRGM_positive1_negative0, data=t2comnegativnodta) %>% 
  tbl_regression(exp = TRUE)
```
RFS t2comnegativnodta -> t2, Comutations without dta MRD-, MRGM MRD pos vs. neg.
```{r}
fit_t2cohort<- survfit(Surv(RFS_years, RFS_status)~t2comnegativnodta$t2_MRD_MRGM_positive1_negative0, data=t2comnegativnodta)
ggsurv <- ggsurvplot(
  fit_t2cohort,                     # survfit object with calculated statistics.
  data = t2comnegativnodta,             # data used to fit survival curves.
  risk.table = TRUE, fontsize = 5,      # show risk table.
...
Calculate the p value with logrank test and Cox regression.
```{r}
survdiff(Surv(RFS_years, RFS_status)~t2comnegativnodta$t2_MRD_MRGM_positive1_negative0, data=t2comnegativnodta)
coxph(Surv(RFS_years, RFS_status) ~ t2comnegativnodta$t2_MRD_MRGM_positive1_negative0, data=t2comnegativnodta) %>% 
  tbl_regression(exp = TRUE)
```
CIR t2comnegativnodta -> t2, Comutations without dta MRD-, MRGM MRD pos vs. neg.
```{r}
ci.all<-cuminc(ftime=t2comnegativnodta$RFS_years,fstatus=t2comnegativnodta$CIR_event,group=t2comnegativnodta$t2_MRD_MRGM_positive1_negative0)
print(ci.all)
# select subset only CIR, no NRM [3:4]
fig<- ggcompetingrisks( fit =ci.all,
gnames = NULL,
...
fine_gray_test <- crr(t2comnegativnodta$RFS_years, t2comnegativnodta$CIR_event == 1, t2comnegativnodta$t2_MRD_MRGM_positive1_negative0)
summary(fine_gray_test)
